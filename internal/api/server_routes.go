...
func (s *Server) setupRoutes() {
	...
	v1.Use(AuthMiddleware(s.accessManager))
	{
		...
		v1.POST("/videos/text-to-video", s.mediaOperationAlias(config.MediaKindVideo, config.MediaCapabilityTextToVideo))
		v1.POST("/videos/image-to-video", s.mediaOperationAlias(config.MediaKindVideo, config.MediaCapabilityImageToVideo))
		v1.POST("/videos/remove-watermark", s.mediaOperationAlias(config.MediaKindVideo, config.MediaCapabilityRemoveWatermark))
		v1.POST("/audio/speech", s.mediaOperationAlias(config.MediaKindAudio, config.MediaCapabilitySpeech))
		v1.POST("/audio/music", s.mediaOperationAlias(config.MediaKindAudio, config.MediaCapabilityMusic))
		v1.POST("/audio/clone", s.mediaOperationAlias(config.MediaKindAudio, config.MediaCapabilityClone))
		v1.POST("/audio/voice-convert", s.mediaOperationAlias(config.MediaKindAudio, config.MediaCapabilityVoiceConvert))
		v1.POST("/audio/transcriptions", s.mediaOperationAlias(config.MediaKindAudio, config.MediaCapabilityTranscribe))
