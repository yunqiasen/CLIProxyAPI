...
func TestForwardStreamNormalizesCloseErrorBeforeWriteAndCancel(t *testing.T) {
	gin.SetMode(gin.TestMode)
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
	c.Request = httptest.NewRequest(http.MethodGet, "/", nil)
	...
	h.ForwardStream(c, recorder, func(err error) {
		...
		CloseError: func() *interfaces.ErrorMessage {
			return &interfaces.ErrorMessage{StatusCode: http.StatusBadGateway, Error: errors.New("raw close detail")}
		},
		NormalizeTerminalError: func(errMsg *interfaces.ErrorMessage) *interfaces.ErrorMessage {
			return &interfaces.ErrorMessage{StatusCode: errMsg.StatusCode, Error: errors.New("sanitized close error")}
		},
		WriteTerminalError: func(errMsg *interfaces.ErrorMessage) { written = errMsg.Error.Error() },
	})
	if written != "sanitized close error" || canceled != "sanitized close error" {
		t.Fatalf("written=%q canceled=%q, want normalized close error", written, canceled)
	}
}
